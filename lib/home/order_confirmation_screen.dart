...
class _OrderConfirmationScreenState extends State<OrderConfirmationScreen> {
  ...
  // TODO: هذا رقم المرحلة الحالية ثابت مؤقتًا (تجريبي). لاحقًا يجي من الـ API
  // الحقيقي حسب حالة الطلب الفعلية (استلمت المطبخ الطلب، خرج المندوب، إلخ).
  static const int currentDeliveryStage = 2; // 0-index: "خرج للتوصيل"
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          widget.isPickup ? 'استلام الطلب' : 'تتبع الطلب #$orderNumber',
          style: const TextStyle(
            color: AppColors.primaryDark,
            fontWeight: FontWeight.w700,
            fontSize: 16,
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text(
                    widget.isPickup ? 'استلمت طلبك الحين' : 'تم، متابعة',
                    style: const TextStyle(
                      fontSize: 15,
                      fontWeight: FontWeight.w700,
                      color: Colors.white,
  ...
  // ===== الأيقونة الدائرية القديمة لحالة الاستلام (بدون تغيير) =====
  ...
  // ===== خريطة حية لتتبع طلب التوصيل =====
  Widget _deliveryLiveMap() {
    // لو ما وصلتنا إحداثيات (نادرًا)، نرجع لنفس الأيقونة القديمة كبديل آمن.
    ...
    // TODO: موقع المندوب تجريبي (وهمي) حاليًا — لازم يجي حي من الـ API
    // الحقيقي (تتبع لحظي عبر WebSocket أو Polling) بدل هذا الإزاحة الثابتة.
      ...
      child: GoogleMap(
        ...
        polylines: {
          Polyline(
            ...
            points: [courierPosition, destination],
  ...
  // ===== مراحل تتبع التوصيل الأربعة (زي التصميم المرجعي) =====
    ...
    return Container(
      ...
      child: Column(
        children: List.generate(deliveryStages.length, (index) {
          final isDone = index < currentDeliveryStage;
          final isCurrent = index == currentDeliveryStage;
                ...
                children: [
                  ...
                  if (!isLast)
                    Container(
                      ...
                      height: 22,
                      color:
                          isDone ? AppColors.green : AppColors.border,
                    ),
                ],
              ),
            ],
    ...
    return Container(
      ...
      child: Row(
        children: [
          IconButton(
            onPressed: () {
              // TODO: ربط اتصال حقيقي برقم المندوب لما يتوفر من الـ API.
            },
